<?php
	...
	while($tmodul = $modul->fetch(PDO::FETCH_ASSOC)) {
		...
		}elseif ($tmodul['id_modul'] == 17) {
			$linkLine			= $tmodul['deskripsi'];
		}elseif ($tmodul['id_modul'] == 18) {
			$logoUAD			= $tmodul['gambar'];
			$judulLogoUAD		= $tmodul['nama'];
		}elseif ($tmodul['id_modul'] == 19) {
			$logoHKPSI			= $tmodul['gambar'];
			$judulLogoHKPSI		= $tmodul['nama'];
		}
