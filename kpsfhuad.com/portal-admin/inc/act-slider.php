...
if (empty($_SESSION['_kode_login__'])) {
    ...
    if ($_GET['act']==="01") {
        ...
        $nama_slider    = htmlspecialchars($_POST['___in_nama_slider_special_KPSFHUAD']);
        ...
        // Gambar
            ...
            $nama_file_unik = seo($nama_slider)."-".rand(10,99).".".$tipe_file2;
        ...
        // SiteMap 1
            ...
            $id_sub_sitemap = 2;
            $loc_1          = $link1."/assets/images/slider/".$gambar;
            $priority_1     = "0.80";
        // SiteMap 1
        ...
        if ($count>0) {
            try{
                $stmt = $pdo->prepare("INSERT INTO $database
                                (urutan,nama_slider,gambar,status,tgl_update,id_sitemap_1)
                                VALUES(:urutan,:nama_slider,:gambar,:status,NOW(),:id_sitemap_1)" );
                        
                $stmt->bindParam(":urutan", $urutan, PDO::PARAM_STR);
                $stmt->bindParam(":nama_slider", $nama_slider, PDO::PARAM_STR);
    ...
    elseif ($_GET['act']==="02") {
        ...
        $id_slider      = htmlspecialchars($_POST['___in_id_slider_special_KPSFHUAD']);
        $urutan         = htmlspecialchars($_POST['___in_urutan_special_KPSFHUAD']);
        $nama_slider    = htmlspecialchars($_POST['___in_nama_slider_special_KPSFHUAD']);
        $seo            = seo($nama_slider)."-".rand(10,99);
        $status         = htmlspecialchars($_POST['___in_status_special_KPSFHUAD']);
        ...
        try {
            ...
            $statement->bindParam(":nama_slider", $nama_slider, PDO::PARAM_STR);
            ...
            if ($count>0) {
                $_SESSION['_msg__']  = "Berhasil";
                echo "<script>window.location = 'ubah-$link-$id_slider'</script>";
                die();
                exit();
            }
        }catch(PDOException $e){
